...
<%
    ...
    void SendAccountRequest(int UserID)
    {
        try
        {
            MySqlConnection connection = ConnectToDB();
            connection.Open();
            string Query = "update tbl_user set is_enabled = 'n' where user_id = " + UserID + "";
            string Query1 = "insert into account_request(user_id,user_email)values("+UserID+",'"+Session["registered_user_email"].ToString()+"')";
            ...
            SendMail("Account Verification","Your account has been sent for verification",new string[] { Session["registered_user_email"].ToString()});
            CloseConnection(ref connection);
        }
        catch (Exception error)
        { }
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(Request["otp"]) && !string.IsNullOrEmpty(Request["token"]))
            {
                ...
                if (connection != null)
                {
                    ...
                    if (number_of_rows == 0)
                    {
                        Response.Write("<script>if(confirm(Session['user_id'])) document.location = 'OTPValidations.aspx';else  document.location = 'OTPValidations.aspx';</script>");
                    ...
                    else
                    {
                        ...
                        SendAccountRequest((int)UserID);
                        DeleteFromOTPTable(token);
                        Response.Write("<script>if(confirm('OTP has been verified')) document.location = 'Login.aspx';else  document.location = 'Login.aspx';</script>");
