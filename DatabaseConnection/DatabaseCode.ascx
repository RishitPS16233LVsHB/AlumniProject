<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="DatabaseCode.ascx.cs" Inherits="AlumniProject.DatabaseConnection.DatabaseCode" %>
    ...
    void InsertIntoUserTechnologyTable(int technologyID)
    {
        try
        {
            if (!string.IsNullOrEmpty(Session["user_id"].ToString()))
            {
                ...
                {
                    connection.Close();
                    Response.Write("<script>alert('server error,please retry');document.location='settings.aspx'</script>");
                }
            }
            else
        ...
        { 
            Response.Write("<script>alert('"+error.Message+"');document.location='settings.aspx'</script>");        
        }
    }
